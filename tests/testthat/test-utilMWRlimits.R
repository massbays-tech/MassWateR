test_that("Checking output format", {
  result <- utilMWRlimits(tst$resdat, tst$accdat, param = 'TP')
...
test_that("Checking error if unit mismatch between results and accuracy file", {
  ...
  expect_error(utilMWRlimits(tst$resdat, tst$accdatchk, param = 'Nitrate'))
})

test_that("Checking output format if parameter not in accuracy file", {
  tst$accdatchk <- tst$accdat %>% 
    filter(!Parameter %in% 'TP')
  expect_warning(expect_s3_class(utilMWRlimits(resdat = tst$resdat, accdat = tst$accdatchk, param = 'TP'), 'tbl_df'))
})

test_that("Check error if parameter not found", {
  expect_error(utilMWRlimits(resdat = tst$resdat, param = 'notfound'))
})
