...
#' 
#' The base map is obtained from the \code{\link[ggmap]{get_stamenmap}} function.  The \code{zoom} value specifies the resolution of the map.  Use higher values to download map tiles with greater resolution, although this increases the download time.  The \code{maptype} argument describes the type of base map to download. Acceptable options include \code{"terrain"}, \code{"terrain-background"}, \code{"terrain-labels"}, \code{"terrain-lines"}, \code{"toner"}, \code{"toner-2010"}, \code{"toner-2011"}, \code{"toner-background"}, \code{"toner-hybrid"}, \code{"toner-labels"}, \code{"toner-lines"}, \code{"toner-lite"}, or \code{"watercolor"}. 
#' 
...
anlzMWRmap<- function(res, param, acc, sit, site = NULL, resultatt = NULL, dtrng = NULL, ptsize = 4, repel = TRUE, labsize = 3, palcol = 'Greens', yscl = c('auto', 'log', 'linear'), crs = 4326, zoom = 11, maptype = 'terrain-background', buffdist = 0.02, northloc = 'tl', scaleloc = 'br', runchk = TRUE, warn = TRUE){
  ...
  bsmap <- suppressMessages(get_stamenmap(bbox = dat_ext, maptype = maptype, zoom = zoom))
