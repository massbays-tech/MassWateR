...
-   **Characteristic Name**: Should match parameter names in the `Simple Parameter` or `WQX Parameter` column of the `paramsMWR()` data
...
Data imported with `readMWRresults()` are also formatted to take care of a few minor issues for downstream analysis. This formatting includes:
...
For all files,  all parameters in the `Parameter` column are converted to `Simple Parameter` in `paramsMWR()` as needed.  This is done automatically using `formMWRacc()` and `formMWRfrecom()` within the import functions.
