...
For both the accuracy and completeness files,  all parameters in the `Parameter` column are converted to `Simple Parameter` in `params()` as needed.  This is done automatically using `form_dqoaccuracy()` and `form_dqocompleteness()` within the import functions.
