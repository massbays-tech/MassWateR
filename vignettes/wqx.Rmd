...
The "Sample Collection Method ID" and "Project ID" columns can be included in the results file, although these are not required to be included when importing the results file with `readMWRresults()`. The function will add placeholder columns with default values if they are not present in the results file. Additional optional columns that can be included in the results file and included in the output file are "Result Comment" and "Local Record ID".  These will not be added to the output file if they are not present in the results. 
