...
In addition to the checks for the results file used in `readMWRresults()`, two additional columns in the results file are needed for WQX output.  The "Sample Collection Method ID" and "Project ID" columns are required and an error will be returned by `tabMWRwqx()` if they are not present.  Additional optional columns that can be included in the results file and included in the output file are "Result Comment" and "Local Record ID". 
