...
Confidence intervals at 95% for the points can be shown when `group = "all"` or `"location"` by setting `confint = TRUE`.

```{r, fig.height = 4, fig.width = 8}
anlzMWRdate(res = resdat, param = "DO", acc = accdat, thresh = "fresh", group = "all", confint = TRUE)
```

Sites can be averaged across location groups using `group = "location"`.  This option requires the site metadata file passed to the `sit` argument and appropriate values for the location groups passed to the `locgroup` argument that match values in the `Location Group` column of the site metadata file. 
...
Results can be grouped by entries in the `Result Atribute` column using `byresultatt = TRUE`.  Values must be supplied to the `resultatt` argument to use this option and they must be valid result attributes from the site metadata file.  For example, sites with E. coli samples can be grouped by `"Dry"` or `"Wet"` conditions.  Filtering by sites first is advised to reduce the amount of data that are plotted. Note that specifying result attributes with `resultatt` and setting `byresultatt = FALSE` will filter the plot data by the result attributes but will not plot the results separately. 
...
Finally, using `addwater = TRUE` will include lines and polygons of natural water bodies from the OpenStreetMap ([OSM](https://www.openstreetmap.org) project, downloaded using the [osmdata](https://docs.ropensci.org/osmdata) package.  The downloaded objects are plotted as simple features on top of the base map specified with the `maptype` argument.  

```{r, fig.height = 7, fig.width = 6}
anlzMWRmap(res = resdat, param = "DO", acc = accdat, sit = sitdat, addwater = T, warn = F)
```

Use `maptype = NULL` to show only the water body objects.

```{r, fig.height = 7, fig.width = 6}
anlzMWRmap(res = resdat, param = "DO", acc = accdat, sit = sitdat, addwater = T, maptype = NULL, warn = F)
```
