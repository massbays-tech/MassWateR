...
The `anlzMWRseason()` function uses the results data as a file path or data frame as input. Outliers can be shown by month using `group = "month"` and as boxplots using `type = "box"`, as below.  A description of the summary statistics shown by a boxplot is provided in the [outlier checks](https://massbays-tech.github.io/MassWateR/articles/outlierchecks.html) vignette.  The relevant threshold lines for freshwater environments are shown.  

```{r, fig.height = 4, fig.width = 8}
anlzMWRseason(res = resdat, param = 'DO', acc = accdat, group = 'month', type = 'box')
```

The same data can also be grouped by week using an integer to specify the week. Note that there can be no common month/day indicating the start of the week between years and an integer is the only way to compare summaries if the results data span multiple years. 

```{r, fig.height = 4, fig.width = 8}
anlzMWRseason(res = resdat, param = 'DO', acc = accdat, group = 'week', type = 'box')
```

If `group = "box"`, points can also be jittered over over the boxplots using `jitter = TRUE`.
...
```{r, fig.height = 4, fig.width = 8}
anlzMWRseason(res = resdat, param = 'DO', acc = accdat, group = 'week', type = 'box', dtrng = c('2021-05-01', '2021-07-31'))
```
...
```{r, fig.height = 4, fig.width = 8}
anlzMWRseason(res = resdat, param = 'E.coli', acc = accdat, group = 'month', yscl = 'linear')
```

Finally, the plots can be filtered by sites and result attributes using the `site` and `resultatt` arguments. These options will filter the results data based on values in the `Monitoring Location ID` and/or `Result Attribute` columns. 
