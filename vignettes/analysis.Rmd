...
```{r, fig.height = 7, fig.width = 6}
anlzMWRmap(res = resdat, param = "DO", acc = accdat, sit = sitdat, addwater = "nhd", dLevel = "low", warn = F)
```

Water body detail as high: 

```{r, fig.height = 7, fig.width = 6}
anlzMWRmap(res = resdat, param = "DO", acc = accdat, sit = sitdat, addwater = "nhd", dLevel = "high", warn = F)
```

Using `addwater = "osm"` will include lines and polygons of natural water bodies from the OpenStreetMap ([OSM](https://www.openstreetmap.org/)) project, downloaded using the [osmdata](https://docs.ropensci.org/osmdata) package. Using `addwater = "nhd"` may be preferred for more accurate display of water bodies in Massachusetts, whereas `addwater = "osm"` will include water bodies outside of the state. Use `addwater = NULL` to suppress both.
