...
Additional layers can also be added to maps created with `anlzMWRmap()`. For example, a watershed shapefile can be imported as a simple features object using the [sf](https://r-spatial.github.io/sf/) package, then added using `geom_sf()` from ggplot. Any warnings about the coordinate system can be ignored.  The water features will also be clipped to the extent of the sites and will likely not show the entire watershed. 
...
# use geom_sf to add watershed
anlzMWRmap(res = resdat, param = 'DO', acc = accdat, sit = sitdat, addwater = "nhd", site = c("SUD-005","SUD-293","SUD-096","SUD-144"), dLevel = 'high') + 
  geom_sf(data = sudburyMWR)
```
