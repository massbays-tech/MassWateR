...
Changing the axis limits can be done two ways.  Note the difference between using `scale_y_continuous()` and `coord_cartesian()`.  Using `scale_y_continuous()` will omit any data beyond the range provided and rescale the boxplots accordingly.  The `coord_cartesian()` function will not omit any data used to create the boxplots in the original plot.    

```{r, fig.height = 4, fig.width = 8}
p + scale_y_continuous(limits = c(0, 8))
