...
Example files included with the package are imported for demonstration in this vignette. Your data import code will look more like the code chunk above. The example files for the package can be viewed by navigating to their location in the package installation. These are available as Excel files.  Running the code below will show where the files are located.  The [Resources tab](/MassWateR/RESOURCES.md) also includes downloadable templates for these files.
...
The example files are provided to demonstrate how the functions in MassWateR are used. In practice, alternative data files that follow the same format as the examples will be used with the functions.  For example, data import in R may look something like the following using actual data files.  Note that all inputs can also be created as a list to pass to any function in MassWateR (see the [very end](https://massbays-tech.github.io/MassWateR/articles/inputs.html#using-the-fset-argument) of this vignette).
