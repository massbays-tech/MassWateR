...
```{r}
tabMWRacc(res= resdat, acc = accdat, type = 'summary', accchk = 'Field Blanks', warn = FALSE)
```
```{r}
tabMWRacc(res= resdat, acc = accdat, type = 'summary', accchk = 'Lab Blanks', warn = FALSE)
```
```{r}
tabMWRacc(res= resdat, acc = accdat, type = 'summary', accchk = 'Field Duplicates', warn = FALSE)
```
```{r}
tabMWRacc(res= resdat, acc = accdat, type = 'summary', accchk = 'Lab Duplicates', warn = FALSE)
```
```{r}
tabMWRacc(res= resdat, acc = accdat, type = 'summary', accchk = 'Lab Spikes', warn = FALSE)
```
```{r}
tabMWRacc(res= resdat, acc = accdat, type = 'summary', accchk = 'Instrument Checks (post sampling)', warn = FALSE)
```

For `type = "summary"`, the `tabMWRacc()` function... The `accchk` argument does not apply for this table and results are returned for all checks.

For `type = "percent"`, the `tabMWRacc()` function... The `accchk` argument does not apply for this table and results are returned for all checks.

The `tabMWRacc()` function uses the `qcMWRacc()` function internally to create the table.  This function creates the raw summaries of accuracy from the input data.  Typically, `qcMWRacc()` is not used by itself because it is used internally by `tabMWRfre()`.  It is explained here to demonstrate how the raw summaries can be obtained, if desired. 
