...
The report can be created as follows by including the required files and specifying an output directory where the Word document is saved (default is a temporary directory).  Once the function is done running, a message indicating success and where the file is located is returned.  The Word file can be further edited by hand as needed.  

```{r, message = T}
qcMWRreview(res = resdat, acc = accdat, frecom = frecomdat, warn = FALSE)
```
